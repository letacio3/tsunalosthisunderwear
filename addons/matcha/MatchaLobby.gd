...
func _init(options:={}):
	...
	_lobby.connection_estabilished.connect(self._on_connection_ok)
	_lobby.disconnected.connect(self._on_disconnected)
# Public methods
...
func create_room(room_meta:={}) -> Error:
	...
	room_meta.leader = _current_room.get_unique_id()
	var room = {
		"id": _current_room.id,
		"meta": room_meta
	}
